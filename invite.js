...
client.on("guildMemberAdd", (member) => {
    ...
    guild.fetchInvites().then(invites => {        var invite = invites.find(_i => gi.has(_i.code) && gi.get(_i.code).uses < _i.uses) || gi.find(_i => !invites.has(_i.code)) || guild.vanityURLCode;
        ...
        if(channel){
       channel.send(new Discord.MessageEmbed().setColor('BLACK').setDescription(`:inbox_tray: **${member.user}** adlı kullanıcı sunucuya katıldı. Kullanıcıyı davet eden **${invite.inviter}** kişisininin \`${total + bonus}\` daveti oldu.`)
                    ) }
    }).catch();
});
    ...
    db.add(`invites.${data.inviter}.leave`, 1);
     if(channel){
        let user = client.users.cache.get(data.inviter)
     	channel.send(new Discord.MessageEmbed().setColor('BLACK').setDescription(`:outbox_tray: **${member.user}** adlı kullanıcı sunucumuzdan ayrıldı. Kullanıcıyı davet eden **${user}** kişisinin \`${Number(total) + Number(bonus)}\` daveti kaldı!`))
     }
});
